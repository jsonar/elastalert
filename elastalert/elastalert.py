...
class ElastAlerter():
    ...
    def get_saved_source_query(conf, saved_source_id, starttime=None, endtime=None, sort=True, timestamp_field='@timestamp', to_ts_func=dt_to_ts, desc=False,
        ...
        endtime = to_ts_func(endtime)
        if starttime and endtime:
            saved_source_query['query']['filtered']['must'].insert(0, {'range': {timestamp_field: {'gt': starttime,
                                                                                                   'lte': endtime}}})
        if five:
            saved_source_query['query']['bool'] = saved_source_query['query'].pop('filtered')

        if sort:
            saved_source_query['sort'] = [{timestamp_field: {'order': 'desc' if desc else 'asc'}}]
        return saved_source_query
